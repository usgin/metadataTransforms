...
<xsl:stylesheet version="1.0"  xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:ows="http://www.opengis.net/ows" xmlns:ows11="http://www.opengis.net/ows/1.1" xmlns:gml="http://www.opengis.net/gml" xmlns:csw="http://www.opengis.net/cat/csw" xmlns:csw202="http://www.opengis.net/cat/csw/2.0.2" xmlns:wcs="http://www.opengis.net/wcs" xmlns:wcs11="http://www.opengis.net/wcs/1.1" xmlns:wcs111="http://www.opengis.net/wcs/1.1.1" xmlns:wfs="http://www.opengis.net/wfs" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:wms="http://www.opengis.net/wms" xmlns:wps100="http://www.opengis.net/wps/1.0.0" xmlns:sos10="http://www.opengis.net/sos/1.0" xmlns:sps="http://www.opengis.net/sps" xmlns:tml="http://www.opengis.net/tml" xmlns:sml="http://www.opengis.net/sensorML/1.0.1" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:myorg="http://www.myorg.org/features" xmlns:swe="http://www.opengis.net/swe/1.0.1" xmlns:exslt="http://exslt.org/common">
	...
	<!-- *********** -->
	<!-- This style sheet converts OGC WFS capabilities response document (versions 1.0.0, 1.1.0 to 
 ...
 and USGIN service metadata example xml document -->
	...
	<xsl:template name="main">
		...
		<gmd:MD_Metadata xmlns="http://www.isotc211.org/2005/gmd" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.isotc211.org/2005/gmd http://schemas.opengis.net/iso/19139/20060504/gmd/gmd.xsd
	http://www.isotc211.org/2005/srv http://schemas.opengis.net/iso/19139/20060504/srv/srv.xsd" xsl:exclude-result-prefixes="ows ows11 wms wps100 swe myorg tml sml sps sos10 wfs wcs wcs11 wcs111 csw csw202 gml">
			...
			<!-- (M-M) Resource type - this is specific to WFS, so define as service -->
			<gmd:hierarchyLevel>
			...
			<gmd:contact>
				<gmd:CI_ResponsibleParty>
					...
					<gmd:contactInfo>
						<gmd:CI_Contact>
							...
							<gmd:contactInstructions>
								<gco:CharacterString>Fill out contact form at
									http://www.azgs.az.gov </gco:CharacterString>
							</gmd:contactInstructions>
						</gmd:CI_Contact>
					</gmd:contactInfo>
					...
					<gmd:role>
						...
						<gmd:CI_RoleCode codeList="http://standards.iso.org/ittf/PubliclyAvailableStandards/ISO_19139_Schemas/resources/Codelist/gmxCodelists.xml#CI_RoleCode" codeListValue="pointOfContact">point of contact</gmd:CI_RoleCode>
					</gmd:role>
				</gmd:CI_ResponsibleParty>
			</gmd:contact>
			...
			<gmd:contact>
				<gmd:CI_ResponsibleParty>
					<gmd:organisationName>
						<gco:CharacterString>Arizona Geological Survey
						</gco:CharacterString>
					</gmd:organisationName>
					<gmd:contactInfo>
						<gmd:CI_Contact>
		...
		<MD_ReferenceSystem>
				...
				<srv:SV_ServiceIdentification>
					<citation>
						<CI_Citation>
							<title>
								<xsl:choose>
									<xsl:when test="string-length(//ows:ServiceIdentification/ows:Title) > 0 or string-length(//ows11:ServiceIdentification/ows11:Title) > 0">
										<gco:CharacterString>
								...
								//ows11:ServiceIdentification/ows11:Title"/>
									...
									<xsl:otherwise>
										<gco:CharacterString>
											<xsl:value-of select="concat('CSW catalog service hosted at ', $sourceUrl)"/>
										</gco:CharacterString>
									</xsl:otherwise>
								</xsl:choose>
							</title>
							...
							<xsl:variable name="hasContactInfo" select="//ows:ServiceContact//text()"/>
							<xsl:choose>
									...
									<gmd:citedResponsibleParty>
										<gmd:CI_ResponsibleParty>
											...
											<gmd:organisationName>
												<gco:CharacterString>
													<xsl:choose>
														<xsl:when test="string-length(//ows:ProviderName) > 0">
											...
											<gmd:contactInfo>
												<gmd:CI_Contact>
													...
													<gmd:address>
														<gmd:CI_Address>
															<xsl:if test="string-length(//ows:Address/ows:DeliveryPoint)>0">
																<gmd:deliveryPoint>
																	<gco:CharacterString>
																		<xsl:value-of select="//ows:Address/ows:DeliveryPoint"/>
																	</gco:CharacterString>
																</gmd:deliveryPoint>
															</xsl:if>
															<xsl:if test="string-length(//ows:Address/ows:City)>0">
																<gmd:city>
																	<gco:CharacterString>
															...
															<xsl:if test="string-length(//ows:Address/ows:AdministrativeArea)>0">
																<gmd:administrativeArea>
																	<gco:CharacterString>
																		<xsl:value-of select="//ows:Address/ows:AdministrativeArea"/>
																	</gco:CharacterString>
																</gmd:administrativeArea>
															</xsl:if>
															<xsl:if test="string-length(//ows:Address/ows:PostalCode)>0">
																<gmd:postalCode>
																	<gco:CharacterString>
																		<xsl:value-of select="//ows:Address/ows:PostalCode"/>
																	</gco:CharacterString>
																</gmd:postalCode>
															</xsl:if>
															<xsl:if test="string-length(//ows:Address/ows:Country)>0">
																<gmd:country>
																	<gco:CharacterString>
															...
															<gmd:electronicMailAddress>
																<xsl:choose>
																	<xsl:when test="string-length(//ows:Address/ows:ElectronicMailAddress)>0">
																		<gco:CharacterString>
																			<xsl:value-of select="//ows:Address/ows:ElectronicMailAddress"/>
																		</gco:CharacterString>
																	</xsl:when>
																	<xsl:otherwise>
					...
					<gmd:abstract>
						<xsl:choose>
							<xsl:when test="string-length(//ServiceIdentification/Abstract)>0">
								<xsl:variable name="theAbstract" select="//ServiceIdentification/Abstract"/>
								<gco:CharacterString>
									<xsl:value-of select="concat($theAbstract,'. [This metadata was harvested from service GetCapabilities response by USGIN GeoPortal catalog.]')"/>
								</gco:CharacterString>
							</xsl:when>
							<xsl:otherwise>
								<gco:CharacterString>OGC WFS service, this metadata harvested
									from service capabilities. No abstract provided by service.
								</gco:CharacterString>
							</xsl:otherwise>
						</xsl:choose>
					</gmd:abstract>
								...
								<gmd:CI_ResponsibleParty>
									<!-- (M-M) (individualName + organisationName + positionName) 
										...
										<gmd:individualName>
											<gco:CharacterString>
											<xsl:choose>
												<xsl:when test="string-length(//ows:IndividualName)>0">									
					...
					<xsl:for-each select="//ows:Keywords">
						<gmd:descriptiveKeywords>
							<gmd:MD_Keywords>
								...
								<xsl:for-each select="//ows:Keyword[not(.=following::ows:Keyword)]">
									<gmd:keyword>
										<gco:CharacterString>
											<xsl:value-of select="."/>
										</gco:CharacterString>
								...
								<!-- WFS capabiltities include a keyword type with a codespace, but no standard codespace is specified.-->
								<xsl:if test="string-length(./ows:Type)>0">
								<gmd:type>
									<gmd:MD_KeywordTypeCode>
									<xsl:choose>
										<xsl:when test="string-length(/ows:Type/@codeSpace)>0">
											<xsl:attribute name="codeList"><xsl:value-of select="./ows:Type/@codeSpace"/></xsl:attribute>
											<xsl:attribute name="codeListValue"><xsl:value-of select="./ows:Type"/></xsl:attribute>
										</xsl:when>
										<xsl:otherwise>
			...
			<xsl:attribute name="codeListValue"><xsl:value-of select="./ows:Type"/></xsl:attribute>
					...
					<srv:serviceType>
						<!-- Valid values for OGC services would be then {<WMS, WFS, WVS, CSW, 
							…} -->
						<gco:LocalName codeSpace="http://resources.usgin.org/uri-gin/usgin/vocabulary/serviceType">WFS</gco:LocalName>
					</srv:serviceType>
			...
			<distributionInfo>
				<MD_Distribution>
					<!-- distribution format -->
					<xsl:for-each select="//ows:OperationsMetadata//ows:Operation[@name='GetRecords']//ows:Parameter[@name='outputFormat']//ows:Value">
						<distributionFormat>
							<MD_Format>
								<name>
									<gco:CharacterString>
						...
						<gmd:MD_DigitalTransferOptions>
							<gmd:onLine>
								<gmd:CI_OnlineResource>
									...
									<!-- Service Description -->
									<gmd:description>
										<gco:CharacterString>Full URL to request the OGC getCapabilities document. This is the mechanism used to acquire detailed operation description for USGIN metadata.</gco:CharacterString>
									</gmd:description>
								</gmd:CI_OnlineResource>
							</gmd:onLine>
							<gmd:onLine>
								<gmd:CI_OnlineResource>
									...
									<!-- Service Description -->
									<gmd:description>
										<gco:CharacterString>Base URL for service access; append '?' and  standard CSW request parameters to compose query.</gco:CharacterString>
									</gmd:description>
								</gmd:CI_OnlineResource>
							</gmd:onLine>
						</gmd:MD_DigitalTransferOptions>
				...
				<gmd:DQ_Scope>
                    ...
                    <gmd:lineage>
                        <gmd:LI_Lineage>
                            <gmd:statement>
                                <gco:CharacterString>
                                    <xsl:value-of select="concat('This metadata record harvested from ', $sourceUrl, '. and transformed to USGIN ISO19139 profile using ogc_wfs_to_usgin_iso_19139.xslt version 1.0')"/>
                                </gco:CharacterString>
                        </gmd:statement>
                        </gmd:LI_Lineage>
                    </gmd:lineage>
