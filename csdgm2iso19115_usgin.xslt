...
======================================================================
	...
	USGIN v1.0
	 ...
	 the Federal Geographic Data Committee 	 June 1998 FGDC-STD-001-1998 version to the ISO 19115:2003(E).  
	...
	more intelligible and maintainable, adopt for USGIN ISO metadata conventions. Spatial reference and entity-attribute information is put into the abstract as free text.
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	exclude-result-prefixes="fn grp vmf xs xsi xsl" xmlns="http://www.opengis.net/gml">
	<!-- ********************** templates by SMR   **************************** -->
	<xsl:template name="usgin:dateFormat">
		...
		<xsl:variable name="var_DateString">
			<xsl:choose>
				<xsl:when
					test="(fn:contains(fn:lower-case(fn:normalize-space(fn:string($inputDate))), 'unknown') or fn:contains(fn:lower-case(fn:normalize-space(fn:string($inputDate))), 'unpublished'))">
				...
				<xsl:when
					test="fn:contains(fn:lower-case(fn:normalize-space(fn:string($inputDate))), 'present')">
					<!--gco:DateTime -->
					...
					<xsl:variable name="var_dateString_result" as="xs:string?">
						<xsl:choose>
							<xsl:when
								...
								<xsl:value-of
									select="fn:concat(fn:substring(fn:normalize-space(fn:string($inputDate)), 0, 5), '-', fn:substring(fn:normalize-space(fn:string($inputDate)), 5, 2), '-',fn:substring(fn:normalize-space(fn:string($inputDate)), 7, 2))"
								/>
							</xsl:when>
							<xsl:when
		...
		<xsl:choose>
			<xsl:when test="starts-with(xs:string($var_DateString),'nil')">
				<xsl:attribute name="gco:nilReason">
					<xsl:value-of select="substring-after($var_DateString,'nil')"/>
				</xsl:attribute>
				<gco:DateTime>1900-01-01T12:00:00</gco:DateTime>
			</xsl:when>
			<xsl:when
				test="(fn:normalize-space(fn:string($var_DateString)) castable as xs:dateTime)">
				<gco:DateTime>
					<xsl:value-of select="fn:string($var_DateString)"/>
				</gco:DateTime>
			</xsl:when>
			<xsl:when
				test="fn:exists($var_DateString) and (fn:string-length($var_DateString)=10) and (fn:string-length($var_timeString)=8)">
				<gco:DateTime>
					<xsl:value-of
			...
			<xsl:when
				test="fn:exists($var_DateString) and (fn:string-length($var_DateString)=10)  and not(fn:string-length($var_timeString)=8)">
				<gco:DateTime>
	...
	<xsl:template name="usgin:contactName">
		...
		<xsl:choose>
			<xsl:when
				test="fn:exists($inputPersonNode) or fn:exists($inputOrganizationNode) or fn:exists($inputPositionNode)">
				...
				<gco:CharacterString>
					<xsl:choose>
						...
						<xsl:otherwise>
							...
							<xsl:value-of
								select="fn:concat(fn:string('http://www.opengis.net/def/nil/OGC/0/missing/'),fn:string(current-dateTime()))"
							/>
						</xsl:otherwise>
					</xsl:choose>
				</gco:CharacterString>
			...
			<gmd:contact>
				<gmd:CI_ResponsibleParty>
					...
					<xsl:variable name="var_contactIndividual_exists" as="xs:string*">
						<xsl:choose>
							<xsl:when
								test="fn:exists($var_metadataRoot/metainfo/metc/cntinfo/cntperp/cntper)">
								<!-- old xsl:when test="fn:exists($var5_map_select_metadata)" -->
								<xsl:for-each
									select="$var_metadataRoot/metainfo/metc/cntinfo/cntperp/cntper">
									<xsl:sequence select="xs:string(.)"/>
								</xsl:for-each>
							</xsl:when>
							<xsl:otherwise>
								<xsl:for-each
									select="$var_metadataRoot/metainfo/metc/cntinfo/cntorgp/cntper">
									<xsl:sequence select="xs:string(.)"/>
								</xsl:for-each>
							</xsl:otherwise>
						</xsl:choose>
					...
					<xsl:variable name="var_contactOrganisation_exists" as="xs:string*">
						<xsl:choose>
							<xsl:when
								test="fn:exists($var_metadataRoot/metainfo/metc/cntinfo/cntperp/cntorg)">
								<xsl:for-each
									select="$var_metadataRoot/metainfo/metc/cntinfo/cntperp/cntorg">
									<xsl:sequence select="xs:string(.)"/>
								</xsl:for-each>
							</xsl:when>
							<xsl:otherwise>
								<xsl:for-each
									select="$var_metadataRoot/metainfo/metc/cntinfo/cntorgp/cntorg">
									<xsl:sequence select="xs:string(.)"/>
								</xsl:for-each>
							</xsl:otherwise>
						</xsl:choose>
					...
					<xsl:call-template name="usgin:contactName">
						<xsl:with-param name="inputPersonNode"
							select="$var_contactIndividual_exists"/>
						<xsl:with-param name="inputOrganizationNode"
							select="($var_contactOrganisation_exists)"/>
						<xsl:with-param name="inputPositionNode"
							select="($var_metadataRoot/metainfo/metc/cntinfo/cntpos)"/>
					</xsl:call-template>
					<gmd:contactInfo>
						<gmd:CI_Contact>
							<!--xsl:variable name="var_eMail_exists" as="xs:string*">
							...
							<xsl:call-template name="usgin:telephoneFormat">
								<xsl:with-param name="telNode"
									select="($var_metadataRoot/metainfo/metc/cntinfo)"/>
							</xsl:call-template>
							<gmd:address>
								<gmd:CI_Address>
									<xsl:call-template name="usgin:addressFormat">
										<xsl:with-param name="inputAddr"
											select="$var_metadataRoot/metainfo/metc/cntinfo/cntaddr[1]"
										/>
									</xsl:call-template>
							...
							<xsl:if test="fn:exists($var_metadataRoot/metainfo/metc/cntinfo/hours)">
								<gmd:hoursOfService>
									...
									<gco:CharacterString>
										<xsl:sequence
											select="fn:normalize-space(xs:string($var_metadataRoot/metainfo/metc/cntinfo/hours))"
										/>
									</gco:CharacterString>
									<!-- /xsl:for-each -->
								</gmd:hoursOfService>
							...
							<xsl:if
								test="fn:exists($var_metadataRoot/metainfo/metc/cntinfo/cntinst)">
								<gmd:contactInstructions>
									<!--xsl:for-each select="" pull this out so don't end up withh multiple characterstring elements -->
									<gco:CharacterString>
										<xsl:sequence
											select="fn:normalize-space(xs:string($var_metadataRoot/metainfo/metc/cntinfo/cntinst))"
										/>
									</gco:CharacterString>
									<!-- /xsl:for-each -->
								</gmd:contactInstructions>
					...
					<xsl:for-each select="rastinfo">
						...
						<gmd:spatialRepresentationInfo>
							<gmd:MD_GridSpatialRepresentation>
								<xsl:variable name="var_rowCount_exists" as="xs:decimal?"
									select="(if (fn:exists(rowcount)) then xs:decimal(1) else ())"/>
								<xsl:variable name="var_colCount_exists" as="xs:decimal?"
									select="(if (fn:exists(colcount)) then xs:decimal(1) else ())"/>
								<xsl:variable name="var_vrtCount_exists" as="xs:decimal?"
									select="(if (fn:exists(vrtcount)) then xs:decimal(1) else ())"/>
								...
								<gmd:axisDimensionProperties>
									<gmd:MD_Dimension>
										<gmd:dimensionName>
											<xsl:variable name="var_hasRow" as="xs:string?"
												select="(if (fn:exists($var_rastinfoNode/rowcount)) then 'row' else ())"/>
								...
								<gmd:axisDimensionProperties>
									<gmd:MD_Dimension>
										<gmd:dimensionName>
											<xsl:variable name="var_hasColumn" as="xs:string?"
												select="(if (fn:exists($var_rastinfoNode/colcount)) then 'column' else ())"/>
								...
								<xsl:if test="fn:exists($var_rastinfoNode/vrtcount)">
									<gmd:axisDimensionProperties>
										<gmd:MD_Dimension>
											<gmd:dimensionName>
												<xsl:variable name="var_hasVertical" as="xs:string?"
												select="(if (fn:exists($var_rastinfoNode/vrtcount)) then 'vertical' else ())"/>
												<gmd:MD_DimensionNameTypeCode
												codeList="http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_DimensionNameTypeCode"
								...
								<xsl:for-each select="vpfterm">
									<gmd:topologyLevel>
										<gmd:MD_TopologyLevelCode>
											<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string(xs:anyURI(fn:normalize-space(fn:string(vpflevel))))"
												/>
											</xsl:attribute>
										</gmd:MD_TopologyLevelCode>
									</gmd:topologyLevel>
									...
									<gmd:geometricObjects>
										<gmd:MD_GeometricObjects>
											<gmd:geometricObjectType>
												...
												<xsl:if
												test="fn:exists($var142_map_select_sdtsterm)">
												<xsl:sequence
												select="fn:string-join($var142_map_select_sdtsterm, ' ')"
												/>
												</xsl:if>
												</xsl:when>
												<xsl:otherwise>
												...
												<xsl:if
												test="fn:exists($var139_result_geometryType2)">
												<xsl:sequence
												select="$var139_result_geometryType2"/>
												</xsl:if>
												</xsl:for-each>
												</xsl:if>
												</xsl:variable>
												...
												<xsl:if
												test="fn:exists((if ((fn:exists($var113_map_select_sdtsterm) or fn:exists($var112_map_select_vpfterm))) then 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_GeometricObjectTypeCode' else ()))">
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI(fn:string((if ((fn:exists($var113_map_select_sdtsterm) or fn:exists($var112_map_select_vpfterm))) then 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_GeometricObjectTypeCode' else ()))))"
												/>
												</xsl:attribute>
												</xsl:if>
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string(xs:anyURI(fn:string($sdtsTermExists)))"
												/>
												</xsl:attribute>
												...
												as="xs:string?">
												<xsl:if
												test="fn:exists($var119_map_select_sdtsterm)">
												...
												<xsl:if
												test="fn:exists($var127_map_select_sdtsterm)">
												<xsl:sequence
												select="fn:string-join($var127_map_select_sdtsterm, ' ')"
												/>
												</xsl:if>
												</xsl:when>
												<xsl:otherwise>
												...
												<xsl:if
												test="fn:exists($var124_result_geometryCode2)">
												<xsl:sequence
												select="$var124_result_geometryCode2"/>
												</xsl:if>
												</xsl:for-each>
												</xsl:if>
												</xsl:variable>
												<xsl:if
												test="fn:exists($var118_cond_result_exists)">
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string(xs:anyURI(fn:string($var118_cond_result_exists)))"
												/>
												</xsl:attribute>
												</xsl:if>
											...
											<gmd:geometricObjectCount>
												...
												<xsl:choose>
												<xsl:when
												test="fn:exists($var152_map_select_sdtsterm)">
												<xsl:for-each select="sdtsterm/ptvctcnt">
												<xsl:sequence select="xs:string(xs:integer(.))"/>
												</xsl:for-each>
												</xsl:when>
							...
							<xsl:for-each select="citation">
								<gmd:citation>
									<xsl:for-each select="citeinfo">
										<xsl:variable name="var_citeinfoSourceNode" as="node()"
											select="."/>
										<gmd:CI_Citation>
											<gmd:title>
											...
											<gmd:date>
												...
												<xsl:variable name="var_pubdate_exists"
												as="xs:string?"
												select="(if (fn:exists(pubdate)) then 'publication' else ())"/>
												<xsl:choose>
											...
											<xsl:for-each
												select="$var_metadataRoot/distinfo/resdesc">
												...
												<gmd:CI_Citation>
												<gmd:title>
												<gco:CharacterString>NOAA NGDC
												convention</gco:CharacterString>
												</gmd:title>
											...
											<gmd:identifier>
												...
												<gco:CharacterString>
												<xsl:for-each
												select="$var_idinfoSourceNode/citation/citeinfo/onlink">
												<xsl:sequence
												select="fn:concat(fn:normalize-space(xs:string(.)),' ')"
												/>
												</xsl:for-each>
											...
											<gmd:citedResponsibleParty>
												...
												<xsl:value-of
												select="xs:integer(fn:count(origin))"/>
												</xsl:variable>
												<xsl:for-each select="origin">
												<xsl:sequence
												select="fn:normalize-space(xs:string(.))"/>
											...
											<gmd:presentationForm>
												...
												<xsl:if
												test="fn:exists($var356_map_select_geoform)">
												<xsl:sequence
												select="xs:string(fn:string-join($var356_map_select_geoform, ' '))"
												/>
												</xsl:if>
												</xsl:when>
												<xsl:otherwise>
												...
												as="xs:string*">
												<xsl:for-each select="geoform"> xs:string( )
												<xsl:sequence
												select="fn:normalize-space(xs:string(.))"/>
												</xsl:for-each>
												</xsl:variable>
												<xsl:if
												test="fn:exists($var360_map_select_geoform)">
												<xsl:sequence
												select="xs:string(fn:string-join($var360_map_select_geoform, ' '))"
												/>
												</xsl:if>
												</xsl:otherwise>
												</xsl:choose>
												...
												<xsl:variable name="var345_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var_citeinfoSourceNode/geoform)) then 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#CI_PresentationFormCode' else ())"/>
												<xsl:if
												test="fn:exists($var345_cond_result_exists)">
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI($var345_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
												<xsl:attribute name="codeListValue">
												...
												as="xs:string*">
												<xsl:for-each
												select="$var_citeinfoSourceNode/geoform">
												...
												<xsl:if
												test="fn:exists($var350_map_select_geoform)">
												<xsl:sequence
												select="xs:string(fn:string-join($var350_map_select_geoform, ' '))"
												/>
												</xsl:if>
												</xsl:if>
												</xsl:variable>
...
An xsl template for displaying metadata in ArcInfo8 with the traditional FGDC look and feel created by mp
	-->
									Other sections: <xsl:for-each
										select="$var_metadataRoot/spdoinfo"> Spatial Data
										Organization: <xsl:for-each select="indspref">
											Indirect_Spatial_Reference_Method: <xsl:value-of
												select="."/>
										</xsl:for-each>
										<xsl:for-each select="direct">
											Direct_Spatial_Reference_Method: <xsl:value-of
												select="."/>
										</xsl:for-each>
										<xsl:for-each select="ptvctinf">
											Point_and_Vector_Object_Information: <xsl:for-each
												select="sdtsterm"> SDTS_Terms_Description:
									...
									<xsl:for-each select="$var_metadataRoot/eainfo">
										Entity_and_Attribute_Information: <xsl:for-each
											...
											<xsl:for-each select="attr"> Attribute: <xsl:for-each
												...
												Attribute_Domain_Values: <xsl:for-each
												select="edom"> Enumerated_Domain: <xsl:for-each
												select="edomv"> Enumerated_Domain_Value:
												<xsl:value-of select="."/>
												</xsl:for-each>
												...
												Attribute_Units_of_Measure: <xsl:value-of
												select="."/>
												</xsl:for-each>
												<xsl:for-each select="attrmres">
												Attribute_Measurement_Resolution: <xsl:value-of
												select="."/>
										...
										<xsl:for-each select="overview"> Overview_Description:
												...
												Entity_and_Attribute_Overview: <xsl:value-of
												select="."/>
											</xsl:for-each>
									...
									<xsl:choose>
										<xsl:when test="fn:exists(status/progress)">
											<xsl:variable name="var_checkStatusProgress"
												...
												<xsl:if
												test="fn:exists($var_checkStatusTypeLookup)">
											...
											<xsl:if test="fn:exists($var_checkStatusProgress)">
												<xsl:sequence
												select="xs:string(fn:string-join($var_checkStatusProgress, ' '))"
												/>
											</xsl:if>
										</xsl:when>
										<xsl:otherwise>
									...
									<gmd:MD_ProgressCode>
										<xsl:attribute name="codeList">
											<xsl:variable name="var381_map_select_status"
												as="xs:string*">
												<xsl:for-each
												select="$var_idinfoSourceNode/status/progress">
												<xsl:sequence select="xs:string(.)"/>
												</xsl:for-each>
											</xsl:variable>
											<xsl:variable name="var380_cond_result_exists"
												as="xs:string"
												select="(if (fn:exists($var381_map_select_status)) then 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_ProgressCode' else '')"/>
											<xsl:sequence
												select="xs:string(xs:anyURI($var380_cond_result_exists))"
											/>
										</xsl:attribute>
										<xsl:attribute name="codeListValue">
											<xsl:sequence select="xs:string(xs:anyURI(.))"/>
										...
										<xsl:variable name="var384_cond_result_exists"
											...
											<xsl:choose>
												<xsl:when
												test="fn:exists($var387_map_select_status)">
												...
												<xsl:if
												test="fn:exists($var392_result_statusCode)">
												<xsl:sequence select="$var392_result_statusCode"/>
												</xsl:if>
												</xsl:for-each>
												</xsl:variable>
												<xsl:if
												test="fn:exists($var389_map_select_status)">
												<xsl:sequence
												select="xs:string(fn:string-join($var389_map_select_status, ' '))"
												/>
												</xsl:if>
												</xsl:when>
												<xsl:otherwise>
								...
								<gmd:pointOfContact>
									...
									<xsl:for-each select="cntinfo">
										...
										<gmd:CI_ResponsibleParty>
											...
											<xsl:call-template name="usgin:contactName">
												<xsl:with-param name="inputPersonNode"
												select="$var_cntIndividualName"/>
												<xsl:with-param name="inputOrganizationNode"
												select="$var_cntOrgName"/>
												<xsl:with-param name="inputPositionNode"
												select="cntpos"/>
											</xsl:call-template>
											<gmd:contactInfo>
							...
							<gmd:resourceMaintenance>
								<gmd:MD_MaintenanceInformation>
									<gmd:maintenanceAndUpdateFrequency>
										...
										<xsl:variable name="var335_cond_result_exists"
											as="xs:string?"
											select="(if (fn:exists($var338_result_vmf11_inputtoresult)) then $var338_result_vmf11_inputtoresult else ())"/>
										<xsl:if test="fn:exists($var335_cond_result_exists)">
											<gmd:MD_MaintenanceFrequencyCode>
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI('http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_MaintenanceFrequencyCode'))"
												/>
												</xsl:attribute>
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string(xs:anyURI($var335_cond_result_exists))"
												/>
												</xsl:attribute>
												<xsl:variable
												name="var337_result_vmf12_inputtoresult"
												as="xs:string?">
												...
												<xsl:variable name="var336_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var337_result_vmf12_inputtoresult)) then $var337_result_vmf12_inputtoresult else ())"/>
												<xsl:if test="fn:exists($var336_cond_result_exists)">
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string(xs:anyURI($var336_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
								...
								<gmd:resourceMaintenance>
									<gmd:MD_MaintenanceInformation>
										<gmd:maintenanceAndUpdateFrequency>
											...
											<xsl:variable name="var341_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var344_result_vmf11_inputtoresult)) then $var344_result_vmf11_inputtoresult else ())"/>
											<xsl:if test="fn:exists($var341_cond_result_exists)">
												...
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string(xs:anyURI($var341_cond_result_exists))"
												/>
												</xsl:attribute>
												<xsl:variable
												name="var343_result_vmf12_inputtoresult"
												as="xs:string?">
												<xsl:call-template name="vmf:updateFrequencyCode">
												<xsl:with-param name="input"
												select="fn:upper-case(fn:normalize-space(xs:string(xs:string(.))))"
												...
												<xsl:variable name="var342_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var343_result_vmf12_inputtoresult)) then $var343_result_vmf12_inputtoresult else ())"/>
												<xsl:if
												test="fn:exists($var342_cond_result_exists)">
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string(xs:anyURI($var342_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
										...
										<gmd:updateScope>
											<gmd:MD_ScopeCode>
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI('http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_ScopeCode'))"
												/>
												</xsl:attribute>
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string(xs:anyURI('attribute'))"/>
												</xsl:attribute>
											</gmd:MD_ScopeCode>
										</gmd:updateScope>
									</gmd:MD_MaintenanceInformation>
									...
									<gmd:MD_BrowseGraphic>
										<gmd:fileName>
											<xsl:for-each-group select="."
												group-by="xs:string(browsen)">
												<xsl:variable name="var349_cur_result_groupby"
												as="xs:string" select="current-grouping-key()"/>
												<gco:CharacterString>
							...
							<xsl:choose>
								<xsl:when test="fn:exists(keywords/theme/themekt)">
									<xsl:for-each-group select="keywords/theme"
										group-by="(if (fn:contains(xs:string(themekt), 'ISO 19115')) then () else xs:string(themekt))">
										<xsl:variable name="var352_cur_result_groupby" as="item()+"
											select="current-group()"/>
										<xsl:variable name="var353_cur_result_groupby"
											as="xs:string" select="current-grouping-key()"/>
										<gmd:descriptiveKeywords>
											<gmd:MD_Keywords>
												<xsl:for-each
												select="$var352_cur_result_groupby/themekey">
												<gmd:keyword>
												<gco:CharacterString>
												<xsl:sequence
												select="fn:normalize-space(xs:string(.))"/>
												...
												<gco:CharacterString>
												<xsl:sequence
												select="fn:normalize-space($var353_cur_result_groupby)"
												/>
												</gco:CharacterString>
												</gmd:title>
												<gmd:date>
							...
							<xsl:choose>
								<xsl:when test="fn:exists(keywords/place/placekt)">
									<xsl:for-each-group select="keywords/place"
										group-by="xs:string(xs:string(placekt))">
										<xsl:variable name="var_placeKeywordList" as="item()+"
											select="current-group()"/>
										<xsl:variable name="var_placeGroupKeyValue" as="xs:string"
											select="current-grouping-key()"/>
										<gmd:descriptiveKeywords>
											<gmd:MD_Keywords>
												<xsl:for-each
												select="$var_placeKeywordList/placekey">
												<gmd:keyword>
												<gco:CharacterString>
												<xsl:sequence
												select="fn:normalize-space(xs:string(.))"/>
												...
												<gco:CharacterString>
												<xsl:sequence
												select="fn:normalize-space($var_placeGroupKeyValue)"
												/>
												</gco:CharacterString>
												</gmd:title>
												<gmd:date>
							...
							<xsl:if
								test="fn:string-length(xs:string(keywords/temporal[1]/tempkey[1]))>0">
								...
								<gmd:resourceConstraints>
									<gmd:MD_SecurityConstraints>
										<gmd:classification>
											...
											<xsl:variable name="var385_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var388_result_securityClassType)) then $var388_result_securityClassType else ())"/>
											<xsl:if test="fn:exists($var385_cond_result_exists)">
												...
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string(xs:anyURI($var385_cond_result_exists))"
												/>
												</xsl:attribute>
												<xsl:variable
												name="var387_result_securityClassCode"
												as="xs:string?">
												...
												<xsl:variable name="var386_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var387_result_securityClassCode)) then $var387_result_securityClassCode else ())"/>
												<xsl:if
												test="fn:exists($var386_cond_result_exists)">
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string(xs:anyURI($var386_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
							...
							<xsl:if test="fn:exists(citation/citeinfo/lworkcit)">
								<xsl:for-each select="citation/citeinfo/lworkcit">
									<xsl:variable name="var_largerWorkCitationNode" as="node()"
										select="."/>
									<gmd:aggregationInfo>
										<gmd:MD_AggregateInformation>
											<gmd:aggregateDataSetName>
												...
												select="citeinfo/pubdate"/>
												<xsl:with-param name="inputTime"
												select="citeinfo/pubtime"/>
												</xsl:call-template>
												</gmd:date>
												<gmd:dateType>
												<xsl:variable name="var647_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists(citeinfo/pubdate)) then 'publication' else ())"/>
												<xsl:if
												test="fn:exists($var647_cond_result_exists)">
												<gmd:CI_DateTypeCode>
												<xsl:variable name="var648_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists(citeinfo/pubdate)) then 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#CI_DateTypeCode' else ())"/>
												<xsl:if
												test="fn:exists($var648_cond_result_exists)">
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI($var648_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string(xs:anyURI($var647_cond_result_exists))"
												/>
												</xsl:attribute>
												<xsl:variable name="var649_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists(citeinfo/pubdate)) then '002' else ())"/>
												<xsl:if
												test="fn:exists($var649_cond_result_exists)">
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string(xs:anyURI($var649_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
												...
												<xsl:variable name="orgCount" as="xs:integer">
												<xsl:value-of
												select="xs:integer(fn:count(citeinfo/origin))"/>
												</xsl:variable>
												<xsl:for-each select="citeinfo/origin">
												<xsl:sequence
												select="fn:normalize-space(xs:string(.))"/>
												...
												<xsl:choose>
												<xsl:when
												test="fn:exists($var672_map_select_citeinfo)">
												...
												<xsl:if
												test="fn:exists($var677_result_statusType)">
												<xsl:sequence select="$var677_result_statusType"/>
												</xsl:if>
												</xsl:for-each>
												</xsl:variable>
												<xsl:if
												test="fn:exists($var674_map_select_citeinfo)">
												<xsl:sequence
												select="xs:string(fn:string-join($var674_map_select_citeinfo, ' '))"
												/>
												</xsl:if>
												</xsl:when>
												<xsl:otherwise>
												...
												as="xs:string*">
												<xsl:for-each
												select="$var_largerWorkCitationNode/citeinfo/geoform">
												<xsl:sequence select="xs:string(.)"/>
												</xsl:for-each>
												</xsl:variable>
												<xsl:variable name="var657_cond_result_exists"
												as="xs:string"
												select="(if (fn:exists($var658_map_select_citeinfo)) then 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_ProgressCode' else '')"/>
												<xsl:sequence
												select="xs:string(xs:anyURI($var657_cond_result_exists))"
												/>
												</xsl:attribute>
												<xsl:attribute name="codeListValue">
												<xsl:sequence select="xs:string(xs:anyURI(.))"/>
												...
												<xsl:choose>
												<xsl:when
												test="fn:exists($var664_map_select_citeinfo)">
												...
												<xsl:if
												test="fn:exists($var669_result_statusCode)">
												<xsl:sequence select="$var669_result_statusCode"/>
												</xsl:if>
												</xsl:for-each>
												</xsl:variable>
												<xsl:if
												test="fn:exists($var666_map_select_citeinfo)">
												<xsl:sequence
												select="xs:string(fn:string-join($var666_map_select_citeinfo, ' '))"
												/>
												</xsl:if>
												</xsl:when>
												<xsl:otherwise>
							...
							<xsl:if test="fn:exists(crossref)">
								<xsl:for-each select="crossref">
									<gmd:aggregationInfo>
										<gmd:MD_AggregateInformation>
											<xsl:variable name="var693_crossref" as="node()"
												select="."/>
											<gmd:aggregateDataSetName>
												<gmd:CI_Citation>
												<gmd:title>
												...
												select="(if (fn:exists(citeinfo/pubdate)) then 'publication' else ())"/>
												<xsl:if
												test="fn:exists($var733_cond_result_exists)">
												<gmd:CI_DateTypeCode>
												<xsl:variable name="var734_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists(citeinfo/pubdate)) then 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#CI_DateTypeCode' else ())"/>
												<xsl:if
												test="fn:exists($var734_cond_result_exists)">
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI($var734_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string(xs:anyURI($var733_cond_result_exists))"
												/>
												</xsl:attribute>
												<xsl:variable name="var735_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists(citeinfo/pubdate)) then '002' else ())"/>
												<xsl:if
												test="fn:exists($var735_cond_result_exists)">
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string(xs:anyURI($var735_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
												...
												<xsl:if
												test="fn:exists($var754_map_select_citeinfo)">
												<xsl:sequence
												select="xs:string(fn:string-join($var754_map_select_citeinfo, ' '))"
												/>
												</xsl:if>
												</xsl:when>
												<xsl:otherwise>
												...
												<xsl:for-each select="citeinfo/geoform">
												<xsl:sequence
												select="fn:normalize-space(xs:string(xs:string(.)))"
												/>
												</xsl:for-each>
												</xsl:variable>
												<xsl:if
												test="fn:exists($var758_map_select_citeinfo)">
												<xsl:sequence
												select="xs:string(fn:string-join($var758_map_select_citeinfo, ' '))"
												/>
												</xsl:if>
												</xsl:otherwise>
												</xsl:choose>
												...
												<gmd:CI_PresentationFormCode>
												<xsl:variable name="var743_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var693_crossref/citeinfo/geoform)) then 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#CI_PresentationFormCode' else ())"/>
												<xsl:if
												test="fn:exists($var743_cond_result_exists)">
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI($var743_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
												<xsl:attribute name="codeListValue">
												...
												as="xs:string?">
												<xsl:if
												test="fn:exists($var693_crossref/citeinfo/geoform)">
												<xsl:variable name="var748_map_select_citeinfo"
												as="xs:string*">
												<xsl:for-each
												select="$var693_crossref/citeinfo/geoform">
												...
												<xsl:if
												test="fn:exists($var748_map_select_citeinfo)">
												<xsl:sequence
												select="xs:string(fn:string-join($var748_map_select_citeinfo, ' '))"
												/>
												</xsl:if>
												</xsl:if>
												</xsl:variable>
											...
											<gmd:associationType>
												...
												as="xs:string?"
												select="(if (fn:exists($var_idinfoSourceNode/crossref/citeinfo/origin)) then 'crossReference' else ())"/>
												<xsl:if test="fn:exists($var767_cond_result_exists)">
												<gmd:DS_AssociationTypeCode>
												<xsl:variable name="var768_cond_result_exists"
												as="xs:string?">
												<xsl:if
												test="fn:exists($var_idinfoSourceNode/crossref/citeinfo/origin)">
												<xsl:variable name="var770_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists(citeinfo/origin)) then 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#DS_AssociationTypeCode' else ())"/>
												<xsl:if
												test="fn:exists($var770_cond_result_exists)">
												...
												<xsl:if
												test="fn:exists($var768_cond_result_exists)">
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI($var768_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string(xs:anyURI($var767_cond_result_exists))"
												/>
												</xsl:attribute>
												<xsl:variable name="var771_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var_idinfoSourceNode/crossref/citeinfo/origin)) then '001' else ())"/>
												<xsl:if
												test="fn:exists($var771_cond_result_exists)">
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string(xs:anyURI($var771_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
...
utilitiesCommunication', concat(' ',string(.)))">
							...
							<xsl:for-each select="native">
								<xsl:variable name="var_nativeEnvironmentNode" as="node()"
									select="."/>
									...
									<xsl:variable name="var574_cond_result_exists" as="xs:string?">
										<xsl:choose>
											<xsl:when
												...
												<xsl:if
												test="fn:exists($var579_map_select_distinfo)">
												<xsl:sequence
									...
									<xsl:for-each select="$var574_cond_result_exists">
										<gco:CharacterString>
											<xsl:variable name="var577_cond_result_exists"
												as="xs:string"
												select="(if (fn:exists($var_metadataRoot/distinfo/techpreq)) then ' Technical Prerequisites: ' else ' ')"/>
											<xsl:sequence
												select="fn:normalize-space(fn:string(fn:concat('Native Dataset Environment: ', xs:string($var_nativeEnvironmentNode), $var577_cond_result_exists, .)))"
											/>
										</gco:CharacterString>
									</xsl:for-each>
								</gmd:environmentDescription>
								...
								<gmd:EX_Extent>
									...
									<gmd:geographicElement>
										...
										<gmd:EX_GeographicBoundingBox>
											<xsl:attribute name="id">
												<xsl:sequence
												select="fn:concat('gbb.', generate-id())"/>
											</xsl:attribute>
											<gmd:westBoundLongitude>
												<gco:Decimal>
												<xsl:choose>
												<xsl:when
												test="spdom/bounding/westbc castable as xs:decimal">
												<xsl:value-of
												select="xs:string(xs:decimal(spdom/bounding/westbc))"
												/>
												</xsl:when>
												<xsl:otherwise>
											...
											<gmd:eastBoundLongitude>
												...
												<xsl:choose>
												<xsl:when
												test="spdom/bounding/eastbc castable as xs:decimal">
												<xsl:value-of
											...
											<gmd:southBoundLatitude>
												...
												<xsl:choose>
												<xsl:when
												test="spdom/bounding/southbc castable as xs:decimal">
												<xsl:value-of
											...
											<gmd:northBoundLatitude>
												...
												<xsl:choose>
												<xsl:when
												test="spdom/bounding/northbc castable as xs:decimal">
												<xsl:value-of
									...
									<xsl:for-each select="spdom/dsgpoly">
										<gmd:geographicElement>
											...
											<xsl:variable name="var582_dsgpoly" as="node()"
												select="."/>
									...
									<xsl:for-each select="timeperd/timeinfo/sngdate">
										<xsl:variable name="var_extentSingleDate" as="node()"
											select="."/>
										<gmd:temporalElement>
											<gmd:EX_TemporalExtent>
												...
												<gmd:extent>
												<xsl:if
												test="(fn:contains(fn:lower-case(fn:normalize-space(fn:string(caldate))), 'unknown') or fn:contains(fn:lower-case(fn:normalize-space(fn:string(caldate))), 'unpublished'))">
												<xsl:attribute name="gco:nilReason">
												<xsl:sequence
												select="xs:string(xs:string(fn:lower-case(fn:normalize-space(fn:string(caldate)))))"
												/>
												</xsl:attribute>
												</xsl:if>
												<gml:TimeInstant>
												<xsl:attribute name="gml:id">
												<xsl:sequence
												select="fn:concat('TInst.',generate-id())"/>
												</xsl:attribute>
												<gml:description>
												<xsl:sequence
												select="xs:string(xs:string($var_metadataRoot/idinfo/timeperd/current))"
												/>
												</gml:description>
												...
												<xsl:choose>
												<xsl:when
												test="$var_TimePos castable as xs:dateTime">
						 ...
						 guess the idea is there might be multple mdattim/sngdate elements?-->
									<xsl:for-each select="timeperd/timeinfo/mdattim/sngdate">
										...
										<gmd:temporalElement>
											<gmd:EX_TemporalExtent>
												...
												<xsl:attribute name="gml:id">
												<xsl:sequence
												select="fn:concat('ATimeInstant.',generate-id())"
												/>
												</xsl:attribute>
												<gml:description>
												<xsl:sequence
												...
												<gml:timePosition>
												<xsl:variable name="var718_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists((if (fn:contains(fn:lower-case(fn:normalize-space(fn:string($var713_sngdate/caldate))), 'present')) then 'now' else ()))) then (if (fn:contains(fn:lower-case(fn:normalize-space(fn:string($var713_sngdate/caldate))), 'present')) then 'now' else ()) else 'unknown')"/>
												<xsl:if
												test="fn:exists($var718_cond_result_exists)">
												<xsl:attribute name="indeterminatePosition">
												<xsl:sequence select="$var718_cond_result_exists"
												/>
												</xsl:attribute>
												</xsl:if>
												<xsl:sequence select="xs:string(xs:string(.))"/>
									...
									<xsl:for-each select="timeperd/timeinfo/rngdates">
										...
										<gmd:temporalElement>
											<xsl:if
												test="(fn:contains(fn:lower-case(fn:normalize-space(fn:string(begdate))), 'unknown') or fn:contains(fn:lower-case(fn:normalize-space(fn:string(begdate))), 'unpublished'))">
												<xsl:attribute name="gco:nilReason">
												<xsl:sequence
												select="xs:string(xs:string(fn:lower-case(fn:normalize-space(fn:string(begdate)))))"
												/>
												</xsl:attribute>
											</xsl:if>
											<gmd:EX_TemporalExtent>
												<xsl:attribute name="id">
												<xsl:sequence
												select="fn:concat('DateRngEx.', generate-id())"/>
												</xsl:attribute>
												<gmd:extent>
												<gml:TimePeriod>
												<xsl:attribute name="gml:id">
												<xsl:sequence
												select="fn:concat('bndTimePeriod.',generate-id())"
												/>
												</xsl:attribute>
												<gml:description>
												<xsl:sequence
												...
												<gml:beginPosition>
												<xsl:if
												test="fn:exists((if (fn:contains(fn:lower-case(fn:normalize-space(fn:string($var_rngdatesNode/begdate))), 'present')) then 'now' else ()))">
												<xsl:attribute name="indeterminatePosition">
												<xsl:sequence
												select="(if (fn:contains(fn:lower-case(fn:normalize-space(fn:string($var_rngdatesNode/begdate))), 'present')) then 'now' else ())"
												/>
												</xsl:attribute>
												</xsl:if>
												<!-- xsl:sequence select="xs:string(xs:string(.))"/ -->
												...
												<gml:endPosition>
												<xsl:if
												test="fn:exists((if (fn:contains(fn:lower-case(fn:normalize-space(fn:string($var_rngdatesNode/enddate))), 'present')) then 'now' else ()))">
												<xsl:attribute name="indeterminatePosition">
												<xsl:sequence
												select="(if (fn:contains(fn:lower-case(fn:normalize-space(fn:string($var_rngdatesNode/enddate))), 'present')) then 'now' else ())"
												/>
												</xsl:attribute>
												</xsl:if>
												<xsl:variable name="var_DateTemp" as="node()">
					...
					<xsl:when test="(fn:exists($var_metadataRoot/eainfo/detailed))">
						<gmd:MD_FeatureCatalogueDescription>
							<gmd:includedWithDataset>
								<gco:Boolean>
									<xsl:variable name="var1068_cond_result_exists" as="xs:decimal"
										select="(if (fn:exists($var_metadataRoot/eainfo/detailed)) then xs:decimal(1) else xs:decimal(0))"/>
									<xsl:sequence
										select="xs:string(fn:boolean($var1068_cond_result_exists))"
									/>
								</gco:Boolean>
							</gmd:includedWithDataset>
							...
							<gmd:featureCatalogueCitation>
								<xsl:variable name="var_eainfoDetailedExists" as="xs:decimal"
									select="(if (fn:exists($var_metadataRoot/eainfo/detailed)) then xs:decimal(1) else xs:decimal(0))"/>
								<xsl:if
									test="fn:exists((if ((xs:string($var_eainfoDetailedExists) = 'false')) then () else 'unknown'))">
									<xsl:attribute name="gco:nilReason">
										<xsl:sequence
											select="xs:string((if ((xs:string($var_eainfoDetailedExists) = 'false')) then () else 'unknown'))"
										/>
									</xsl:attribute>
								</xsl:if>
								<xsl:variable name="var_eainfoEaoverLabel" as="xs:string?"
									select="(if (fn:exists($var_metadataRoot/eainfo/overview/eaover)) then 'Entity and Attribute Overview: ' else ())"/>
								<xsl:if test="fn:exists($var_eainfoEaoverLabel)">
									...
									<xsl:for-each select="$var_eainfoEaoverText">
										...
										<xsl:variable name="var_eainfoEadetcitLabel" as="xs:string?"
											select="(if (fn:exists($var_metadataRoot/eainfo/overview/eadetcit)) then '   Entity and Attribute Detail Citation: ' else ())"/>
							...
							<gmd:MD_ImageDescription>
								<gmd:cloudCoverPercentage>
									<xsl:choose>
										...
										<xsl:when
											test="xs:string($var_metadataRoot/dataqual/cloud[1]) castable as xs:decimal">
											<gco:Real>
												<xsl:value-of
				...
				<gmd:MD_Distribution>
						...
						<gmd:distributionFormat>
							<gmd:MD_Format>
								<gmd:name>
									<gco:CharacterString>
										<xsl:sequence
											select="fn:normalize-space(xs:string(digtinfo/formname))"
										/>
									</gco:CharacterString>
								</gmd:name>
								<gmd:version>
									...
									<xsl:for-each select="digtinfo/formvern">
										<xsl:variable name="var1099_cond_result_equal"
											as="xs:string?">
											<xsl:if
												test="not((fn:lower-case(xs:string(.)) = 'unknown'))">
												<xsl:variable name="var1103_map_select_formverd"
												as="xs:string?">
												<xsl:for-each
												select="$var1090_digform/digtinfo/formverd">
												...
												<xsl:choose>
												<xsl:when
												test="fn:exists($var1103_map_select_formverd)">
												<xsl:for-each
												select="$var1090_digform/digtinfo/formverd">
												<xsl:sequence select="fn:string(.)"/>
												</xsl:for-each>
												...
												<xsl:if
												test="fn:exists($var1102_cond_result_exists)">
												<xsl:sequence
												select="fn:normalize-space(fn:string(fn:concat(xs:string(.), $var1102_cond_result_exists)))"
												/>
												</xsl:if>
											</xsl:if>
										</xsl:variable>
								...
								<gmd:distributorContact>
									<gmd:CI_ResponsibleParty>
										<xsl:variable name="var_contactIndividual_exists"
											...
											<xsl:choose>
												<xsl:when
												test="fn:exists(distrib/cntinfo/cntperp/cntper)">
												<!-- old xsl:when test="fn:exists($var5_map_select_metadata)" -->
												<xsl:for-each
												select="distrib/cntinfo/cntperp/cntper">
												<xsl:sequence select="xs:string(.)"/>
												</xsl:for-each>
												</xsl:when>
												<xsl:otherwise>
												<xsl:for-each
												select="distrib/cntinfo/cntorgp/cntper">
												<xsl:sequence select="xs:string(.)"/>
												</xsl:for-each>
												</xsl:otherwise>
											</xsl:choose>
										...
										<xsl:variable name="var_contactOrganisation_exists"
											...
											<xsl:choose>
												<xsl:when
												test="fn:exists(distrib/cntinfo/cntperp/cntorg)">
												<xsl:for-each
												select="distrib/cntinfo/cntperp/cntorg">
												<xsl:sequence select="xs:string(.)"/>
												</xsl:for-each>
												</xsl:when>
												<xsl:otherwise>
												<xsl:for-each
												select="distrib/cntinfo/cntorgp/cntorg">
												<xsl:sequence select="xs:string(.)"/>
												</xsl:for-each>
												</xsl:otherwise>
											</xsl:choose>
										...
										<gmd:contactInfo>
											<gmd:CI_Contact>
												...
												<xsl:call-template name="usgin:addressFormat">
												<xsl:with-param name="inputAddr"
												select="distrib/cntinfo/cntaddr[1]"/>
												</xsl:call-template>
												<!-- done with postal address elements, now the e-mail. Set this up so that some value gets inserted -->
												<xsl:call-template name="usgin:emailFormat">
												<xsl:with-param name="emailNode"
									...
									<gmd:distributionOrderProcess>
										<gmd:MD_StandardOrderProcess>
											<gmd:fees>
												<gco:CharacterString>
												<xsl:sequence
												select="fn:normalize-space(xs:string(fees))"/>
												</gco:CharacterString>
											</gmd:fees>
											...
											<xsl:for-each select="ordering">
												<xsl:variable name="var1542_ordering" as="node()"
												select="."/>
												...
												<xsl:choose>
												<xsl:when
												test="fn:exists($var1159_stdorder/digform/digtopt/onlinopt/accinstr)">
												<xsl:variable name="var1559_map_select_digform"
												as="xs:string*">
												<xsl:for-each
												select="$var1159_stdorder/digform/digtopt/onlinopt/accinstr">
												<xsl:sequence select="xs:string(.)"/>
												</xsl:for-each>
												</xsl:variable>
												<xsl:if
												test="fn:exists($var1559_map_select_digform)">
												<xsl:sequence
												select="fn:string-join($var1559_map_select_digform, ' ')"
												/>
												</xsl:if>
												</xsl:when>
												<xsl:otherwise>
												...
												<xsl:if
												test="fn:exists($var1547_cond_result_exists)">
												...
												<xsl:if
												test="fn:exists($var1548_cond_result_exists)">
												<gco:CharacterString>
												<xsl:variable name="var1549_cond_result_exists"
												as="xs:string"
												select="(if (fn:exists($var1159_stdorder/digform/digtopt/onlinopt/accinstr)) then 'Access Instructions: ' else ' ')"/>
												<xsl:variable name="var1550_cond_result_exists"
												as="xs:string"
												select="(if (fn:exists($var1159_stdorder/nondig)) then 'Non-Digital Form: ' else ' ')"/>
												<xsl:variable name="var1551_cond_result_exists"
												as="xs:string"
												select="(if (fn:exists($var1113_distinfo/custom)) then ' Custom Order Process: ' else ' ')"/>
												<xsl:sequence
												select="fn:normalize-space(fn:string(fn:concat(fn:concat(fn:concat(fn:concat(fn:concat(fn:concat(fn:concat(fn:concat(fn:concat(fn:concat($var1549_cond_result_exists, .), $var1550_cond_result_exists), ' '), $var1547_cond_result_exists), ' '), 'Ordering Instructions: '), ' '), xs:string($var1542_ordering)), $var1551_cond_result_exists), $var1548_cond_result_exists)))"
												/>
												</gco:CharacterString>
												</xsl:if>
												</xsl:if>
					...
					<gmd:transferOptions>
						<gmd:MD_DigitalTransferOptions>
							<xsl:for-each
								select="$var_metadataRoot/distinfo/stdorder/digform/digtinfo/transize">
								<gmd:transferSize>
									<gco:Real>
							...
							<xsl:variable name="var_networkResourceName" as="xs:string*">
								<xsl:for-each
									select="$var_metadataRoot/distinfo/stdorder/digform/digtopt/onlinopt/computer/networka/networkr">
							...
							<xsl:variable name="var_OnlineName" as="xs:string*">
								<xsl:value-of
									select="concat((if (fn:exists($var_ResourceIdentifer)) then $var_ResourceIdentifer else 'NoResdesc'),									
								...
								<gmd:onLine>
									<gmd:CI_OnlineResource>
										...
										<gmd:name>
											<gco:CharacterString>
												<xsl:sequence
												select=" concat($var_OnlineName,substring(.,string-length(.)-3, 4))"/>
					...
					<xsl:for-each-group
						select="$var_metadataRoot/distinfo/stdorder/digform/digtopt/offoptn"
						group-by="xs:string(offmedia)">
						<xsl:variable name="var_offlineMediaOptionGroup" as="item()+"
							select="current-group()"/>
						<gmd:transferOptions>
							<gmd:MD_DigitalTransferOptions>
								<gmd:offLine>
									<gmd:MD_Medium>
										<gmd:name>
											<xsl:variable name="var1618_map_result_groupitems"
												as="xs:string+">
												<xsl:for-each select="$var_offlineMediaOptionGroup">
												<xsl:sequence
												select="fn:normalize-space(xs:string(xs:string(offmedia)))"
												/>
												</xsl:for-each>
											</xsl:variable>
											<xsl:variable name="var1617_map_result_distinctvalues"
												as="xs:string*">
												<xsl:for-each
												select="fn:distinct-values($var1618_map_result_groupitems)">
												<xsl:variable name="var1621_result_mediumType"
												as="xs:string?">
												<xsl:call-template name="vmf:mediumType">
												<xsl:with-param name="input"
												select="fn:upper-case(.)"/>
												</xsl:call-template>
												</xsl:variable>
												<xsl:if
												test="fn:exists($var1621_result_mediumType)">
												<xsl:sequence select="$var1621_result_mediumType"
												/>
												</xsl:if>
												</xsl:for-each>
											</xsl:variable>
											<xsl:variable name="var1608_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var1617_map_result_distinctvalues)) then fn:string-join($var1617_map_result_distinctvalues, ' ') else ())"/>
											<xsl:variable name="var1581_cond_result_exists"
												as="xs:string?">
												<xsl:if test="$var1608_cond_result_exists">
												<xsl:variable name="var1611_map_result_groupitems"
												as="xs:string+">
												<xsl:for-each
												select="$var_offlineMediaOptionGroup">
												<xsl:sequence
												select="fn:normalize-space(xs:string(xs:string(offmedia)))"
												/>
												</xsl:for-each>
												...
												<xsl:variable
												name="var1610_map_result_distinctvalues"
												as="xs:string*">
												<xsl:for-each
												select="fn:distinct-values($var1611_map_result_groupitems)">
												...
												<xsl:if
												test="fn:exists($var1614_result_mediumType)">
												...
												<xsl:if
												test="fn:exists($var1610_map_result_distinctvalues)">
												<xsl:sequence
												select="fn:string-join($var1610_map_result_distinctvalues, ' ')"
												/>
												</xsl:if>
												</xsl:if>
											</xsl:variable>
											<xsl:if test="fn:exists($var1581_cond_result_exists)">
												...
												<xsl:variable name="var1585_map_result_groupitems"
												as="xs:string+">
												<xsl:for-each
												select="$var_offlineMediaOptionGroup">
												<xsl:sequence
												...
												<xsl:variable
												name="var1584_map_result_distinctvalues"
												as="xs:string*">
												<xsl:for-each
												select="fn:distinct-values($var1585_map_result_groupitems)">
												...
												<xsl:if
												test="fn:exists($var1588_result_mediumType)">
												<xsl:sequence select="$var1588_result_mediumType"
												/>
												</xsl:if>
												</xsl:for-each>
												</xsl:variable>
												<xsl:variable name="var1583_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var1584_map_result_distinctvalues)) then fn:string-join($var1584_map_result_distinctvalues, ' ') else ())"/>
												<xsl:variable name="var1582_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var1583_cond_result_exists)) then 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_MediumNameCode' else ())"/>
												<xsl:if
												test="fn:exists($var1582_cond_result_exists)">
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI($var1582_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string(xs:anyURI($var1581_cond_result_exists))"
												/>
												</xsl:attribute>
												<xsl:variable name="var1602_map_result_groupitems"
												as="xs:string+">
												<xsl:for-each
												select="$var_offlineMediaOptionGroup">
												<xsl:sequence
												...
												<xsl:variable
												name="var1601_map_result_distinctvalues"
												as="xs:string*">
												<xsl:for-each
												select="fn:distinct-values($var1602_map_result_groupitems)">
												...
												<xsl:if
												test="fn:exists($var1605_result_mediumType)">
												<xsl:sequence select="$var1605_result_mediumType"
												/>
												</xsl:if>
												</xsl:for-each>
												</xsl:variable>
												<xsl:variable name="var1592_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var1601_map_result_distinctvalues)) then fn:string-join($var1601_map_result_distinctvalues, ' ') else ())"/>
												<xsl:variable name="var1591_cond_result_exists"
												as="xs:string?">
												<xsl:if test="$var1592_cond_result_exists">
												<xsl:variable name="var1595_map_result_groupitems"
												as="xs:string+">
												<xsl:for-each
												select="$var_offlineMediaOptionGroup">
												<xsl:sequence
												...
												<xsl:variable
												name="var1594_map_result_distinctvalues"
												as="xs:string*">
												<xsl:for-each
												select="fn:distinct-values($var1595_map_result_groupitems)">
												...
												<xsl:if
												test="fn:exists($var1598_result_mediumCode)">
												...
												<xsl:if
												test="fn:exists($var1594_map_result_distinctvalues)">
												<xsl:sequence
												select="fn:string-join($var1594_map_result_distinctvalues, ' ')"
												/>
												</xsl:if>
												</xsl:if>
												</xsl:variable>
												<xsl:if
												test="fn:exists($var1591_cond_result_exists)">
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string(xs:anyURI($var1591_cond_result_exists))"
												/>
												</xsl:attribute>
												</xsl:if>
										...
										<xsl:for-each
											select="$var_offlineMediaOptionGroup/reccap/recden">
											<gmd:density>
												<gco:Real>
												<xsl:sequence select="xs:string(xs:double(.))"/>
												</gco:Real>
										...
										<gmd:mediumFormat>
											<xsl:variable name="var1638_map_result_groupitems"
												as="xs:string*">
												<xsl:for-each
												select="$var_offlineMediaOptionGroup/recfmt">
												...
												<xsl:if
												test="fn:exists($var1641_result_encodingType)">
												<xsl:sequence
												select="$var1641_result_encodingType"/>
												</xsl:if>
												</xsl:for-each>
											</xsl:variable>
											<xsl:variable name="var1628_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var1638_map_result_groupitems)) then fn:string-join($var1638_map_result_groupitems, ' ') else ())"/>
											<xsl:for-each select="$var1628_cond_result_exists">
												<gmd:MD_MediumFormatCode>
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI('http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_MediumFormatCode'))"
												/>
												</xsl:attribute>
												<xsl:attribute name="codeListValue">
												<xsl:sequence select="xs:string(xs:anyURI(.))"/>
												...
												<xsl:if
												test="fn:exists($var1637_result_encodingCode)">
												<xsl:sequence
												select="$var1637_result_encodingCode"/>
												</xsl:if>
												</xsl:for-each>
												</xsl:variable>
												<xsl:variable name="var1631_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var1634_map_result_groupitems)) then fn:string-join($var1634_map_result_groupitems, ' ') else ())"/>
										...
										<xsl:variable name="var1642_cond_result_exists"
											as="xs:string?"
											select="(if (fn:exists($var_metadataRoot/distinfo/stdorder/digform/digtinfo/formcont)) then 'Format Information Content:  ' else ())"/>
										<xsl:if test="fn:exists($var1642_cond_result_exists)">
											<xsl:for-each
												select="$var_metadataRoot/distinfo/stdorder/digform/digtinfo/formcont">
												<xsl:variable name="var1643_formcont" as="node()"
												select="."/>
												<xsl:variable name="var1645_cond_result_exists"
												as="xs:string?"
												select="(if (fn:exists($var_offlineMediaOptionGroup/compat)) then ' Compatibility Information: ' else ())"/>
												<xsl:if
												test="fn:exists($var1645_cond_result_exists)">
												<xsl:for-each
												select="$var_offlineMediaOptionGroup/compat">
												<gmd:mediumNote>
												<gco:CharacterString>
												<xsl:sequence
												select="fn:concat(fn:concat(fn:concat($var1642_cond_result_exists, fn:normalize-space(xs:string($var1643_formcont))), $var1645_cond_result_exists), xs:string(.))"
												/>
												</gco:CharacterString>
												</gmd:mediumNote>
												</xsl:for-each>
				...
				<xsl:for-each select="dataqual">
					<gmd:dataQualityInfo>
						<gmd:DQ_DataQuality>
									...
									<gmd:report>
										<gmd:DQ_AbsoluteExternalPositionalAccuracy>
											<gmd:nameOfMeasure>
												<gco:CharacterString>
												<xsl:sequence
												select="'Horizontal Positional Accuracy'"/>
												</gco:CharacterString>
											</gmd:nameOfMeasure>
											<gmd:measureDescription>
												...
												<xsl:value-of
												select="xs:integer(fn:count(/metadata/dataqual/posacc/horizpa/qhorizpa/horizpae))"
												/>
												</xsl:variable>
												<xsl:for-each
												select="/metadata/dataqual/posacc/horizpa/qhorizpa[1]/horizpae">
												<xsl:sequence
												select="fn:normalize-space(xs:string(.))"/>
											...
											<gmd:result>
												...
												<xsl:attribute name="gml:id">
												<xsl:sequence
												select="fn:concat('uom.',generate-id())"/>
												</xsl:attribute>
												<gml:identifier>
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												...
												<xsl:sequence
												select="xs:string(xs:anyURI('http://www.bipm.org/en/si/'))"
												/>
												</xsl:attribute>
												</gml:unitsSystem>
												</gml:BaseUnit>
												...
												<gco:Record>
												<xsl:value-of
												select="xs:string(xs:double(/metadata/dataqual/posacc/horizpa/qhorizpa[1]/horizpav[1]))"
												/>
												</gco:Record>
												</gmd:value>
												</gmd:DQ_QuantitativeResult>
									...
									<gmd:report>
										<gmd:DQ_AbsoluteExternalPositionalAccuracy>
											...
											<gmd:measureDescription>
												...
												<xsl:variable name="DQMeasureDesc" as="xs:integer">
												<xsl:value-of
												select="xs:integer(fn:count(/metadata/dataqual/posacc/vertacc/qvertpa/vertacce[1]))"
												/>
												</xsl:variable>
												<xsl:for-each
											...
											<gmd:result>
												...
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string(xs:anyURI('meters'))"/>
												</xsl:attribute>
												</gml:identifier>
												<gml:unitsSystem>
												<xsl:attribute name="xlink:href">
												...
												<gco:Record>
												<xsl:value-of
												select="xs:string(/metadata/dataqual/posacc/vertacc/qvertpa[1]/vertaccv[2])"
												/>
												</gco:Record>
												</gmd:value>
												</gmd:DQ_QuantitativeResult>
							...
							<xsl:if
								test="fn:exists($var_metadataRoot/eainfo/detailed) or fn:exists(complete)">
								<gmd:report>
									<gmd:DQ_CompletenessOmission>
										...
										<gmd:result>
											<xsl:variable name="var1664_cond_result_exists"
												as="xs:decimal"
												select="(if (fn:exists($var_metadataRoot/eainfo/detailed)) then xs:decimal(1) else xs:decimal(0))"/>
											<xsl:if
												test="fn:exists((if ((xs:string($var1664_cond_result_exists) = 'false')) then () else 'unknown'))">
												<xsl:attribute name="gco:nilReason">
												<xsl:sequence
												select="xs:string(xs:string((if ((xs:string($var1664_cond_result_exists) = 'false')) then () else 'unknown')))"
												/>
												</xsl:attribute>
											</xsl:if>
										</gmd:result>
							...
							<xsl:for-each select="attracc">
								<xsl:variable name="var_attributeAccuracyNode" as="node()"
									select="."/>
									...
									<xsl:if test="fn:exists(attraccv)">
										<gmd:report>
											<gmd:DQ_QuantitativeAttributeAccuracy>
												...
												<gmd:evaluationMethodDescription>
												<xsl:for-each
												select="$var_attributeAccuracyNode/attraccr">
												<gco:CharacterString>
												<xsl:sequence select="xs:string(.)"/>
												</gco:CharacterString>
												</xsl:for-each>
												...
												as="xs:integer">
												<xsl:value-of
												select="xs:integer(fn:count(/metadata/dataqual/attracc/qattracc/attraccv))"
												/>
												</xsl:variable>
												<xsl:for-each
												select="/metadata/dataqual/attracc/qattracc/attraccv">
												<xsl:sequence
												select="fn:normalize-space(xs:string(.))"/>
								...
								<gmd:lineage>
									<gmd:LI_Lineage>
										...
										<xsl:for-each select="lineage/procstep">
											<xsl:variable name="var_lineageProcessingStepNode"
												as="node()" select="."/>
											<gmd:processStep>
												<gmd:LI_ProcessStep>
												<gmd:description>
												...
												<xsl:variable
												name="var_contactOrganisation_exists"
												as="xs:string*">
												<xsl:choose>
												...
												<xsl:for-each-group select="."
												group-by="grp:sourcesUsed(.)">
												<xsl:variable name="var1800_cur_result_groupby"
												as="item()+" select="current-group()"/>
												<xsl:variable name="var1801_cur_result_groupby"
												as="xs:string" select="current-grouping-key()"/>
												<gmd:source>
												<gmd:LI_Source>
												<gmd:sourceCitation>
												...
												<xsl:for-each
												select="$var1800_cur_result_groupby/srcprod">
												<gmd:alternateTitle>
												<gco:CharacterString>
												<xsl:sequence select="xs:string(.)"/>
												</gco:CharacterString>
										...
										<xsl:for-each select="lineage/srcinfo">
											<xsl:variable name="var_lineageSourceInfoNode"
												as="node()" select="."/>
											<gmd:source>
												<gmd:LI_Source>
												<xsl:if
												...
												<gco:CharacterString>
												<xsl:sequence
												select="fn:concat('Source Contribution: ', xs:string(srccontr), ' ', xs:string(typesrc))"
												/>
												</gco:CharacterString>
												</gmd:description>
												</xsl:if>
												<xsl:if
												test="fn:exists(srcscale) and ((fn:lower-case(fn:string(srcscale)) = 'unknown') or (srcscale castable as xs:integer))">
												...
												<xsl:otherwise>
												<xsl:attribute name="gco:nilReason">
												<xsl:sequence select="xs:string('missing')"/>
												</xsl:attribute>
												</xsl:otherwise>
												</xsl:choose>
												</gmd:denominator>
												...
												<gco:CharacterString>
												<xsl:sequence
												select="xs:string(srccite/citeinfo/title)"/>
												</gco:CharacterString>
												</gmd:title>
												...
												<xsl:call-template name="usgin:dateFormat">
												<xsl:with-param name="inputDate"
												select="srccite/citeinfo/pubdate"/>
												<xsl:with-param name="inputTime"
												select="srccite/citeinfo/pubtime"/>
												</xsl:call-template>
												</gmd:date>
												<gmd:dateType>
												<gmd:CI_DateTypeCode>
												<xsl:attribute name="codeList">
												<xsl:sequence
												select="xs:string(xs:anyURI('http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#CI_DateTypeCode'))"
												/>
												</xsl:attribute>
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string(xs:anyURI('publication'))"/>
												</xsl:attribute>
												<xsl:attribute name="codeSpace">
												...
												<gco:CharacterString>
												<xsl:sequence
												select="xs:string(srccite/citeinfo/edition)"/>
												</gco:CharacterString>
												</gmd:edition>
												</xsl:if>
												<xsl:for-each-group select="srccite/citeinfo"
												group-by="grp:origins(.)">
												<xsl:variable name="var1881_cur_result_groupby"
												as="xs:string" select="current-grouping-key()"/>
												<gmd:citedResponsibleParty>
												<gmd:CI_ResponsibleParty>
												<gmd:organisationName>
												...
												<xsl:if
												test="fn:exists(srccite/citeinfo/geoform) ">
												<gmd:presentationForm>
												<xsl:variable name="var1883_cond_result_exists"
												as="xs:string?">
												<xsl:choose>
												<xsl:when
												test="$var_lineageSourceInfoNode/srccite/citeinfo/geoform">
												<!--xsl:for-each select="srccite/citeinfo/geoform" -->
												<xsl:variable name="var_sourceDocTypeLookup"
												as="xs:string?">
												<xsl:call-template name="vmf:docType">
												<xsl:with-param name="input"
												select="fn:upper-case(fn:normalize-space(xs:string(srccite/citeinfo/geoform)))"
												/>
												</xsl:call-template>
												</xsl:variable>
												...
												<xsl:attribute name="codeListValue">
												<xsl:sequence
												select="xs:string($var1883_cond_result_exists)"/>
												</xsl:attribute>
												<xsl:variable
												name="var_presentationFormCodespaceLookup"
												as="xs:string?">
												<xsl:call-template name="vmf:docCode">
												<xsl:with-param name="input"
												select="fn:upper-case(fn:normalize-space(xs:string(srccite/citeinfo/geoform)))"
												...
												<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string($var_presentationFormCodespaceLookup)"
												/>
												</xsl:attribute>
												...
												<xsl:if
												test="fn:exists(srccite/citeinfo/othercit)">
												<gmd:otherCitationDetails>
												<!--xsl:for-each select="srccite/citeinfo/othercit" -->
												<gco:CharacterString>
												<xsl:sequence
												select="xs:string(srccite/citeinfo/othercit)"/>
												</gco:CharacterString>
												<!-- /xsl:for-each -->
												</gmd:otherCitationDetails>
												</xsl:if>
												...
												<gml:description>
												<xsl:sequence
												select="xs:string(xs:string($var_lineageSourceInfoNode/srctime/srccurr))"
												/>
												</gml:description>
												<xsl:variable name="var_DateTemp" as="node()">
												<xsl:call-template name="usgin:dateFormat">
												...
												<xsl:for-each
												select="srctime/timeinfo/mdattim/sngdate">
												<gmd:temporalElement>
												<gmd:EX_TemporalExtent>
												<gmd:extent>
												<xsl:if
												...
												<xsl:attribute name="gml:id">
												<xsl:sequence
												select="fn:concat('sourceTimeInst.',generate-id())"
												/>
												</xsl:attribute>
												<gml:description>
												<xsl:sequence
												...
												<xsl:variable name="var_rngdatesNode" as="node()"
												select="."/>
												<gmd:temporalElement>
												<gmd:EX_TemporalExtent>
												<gmd:extent>
												...
												<xsl:attribute name="gml:id">
												<xsl:sequence
												select="fn:concat('srcTempEx.',generate-id())"/>
												</xsl:attribute>
												<gml:description>
												<xsl:sequence
												select="xs:string(xs:string($var_lineageSourceInfoNode/srctime/srccurr))"
												...
												<xsl:otherwise> lineab <xsl:attribute
												name="indeterminatePosition"><xsl:value-of
												select="xs:string('unknown')"/></xsl:attribute>
												</xsl:otherwise>
												</xsl:choose>
												</xsl:for-each>
												</gml:endPosition>
								...
								<xsl:if test="fn:exists($var2378_cond_result_exists)">
									<gco:CharacterString>
										<xsl:variable name="var2379_cond_result_exists"
											as="xs:string"
											select="(if (fn:exists($var2374_metadata/metainfo/metuc)) then ' Metadata Use Constraints: ' else ' ')"/>
										<xsl:sequence
											select="fn:normalize-space(fn:concat(fn:concat(fn:concat('Metadata Access Constraints: ', xs:string(.)), $var2379_cond_result_exists), $var2378_cond_result_exists))"
										/>
									</gco:CharacterString>
								</xsl:if>
							</gmd:otherConstraints>
							...
							<xsl:when
								...
								<gmd:classification>
									<gmd:MD_ClassificationCode>
										...
										<xsl:variable name="var2387_cond_result_exists"
											as="xs:string?"
											select="(if (fn:exists($var2388_result_securityClassCode)) then $var2388_result_securityClassCode else ())"/>
										<xsl:if test="fn:exists($var2387_cond_result_exists)">
											<xsl:attribute name="codeSpace">
												<xsl:sequence
												select="xs:string($var2387_cond_result_exists)"/>
											</xsl:attribute>
										</xsl:if>
									</gmd:MD_ClassificationCode>
								</gmd:classification>
				...
				<gmd:MD_MaintenanceInformation>
					...
					<xsl:for-each select="$var_metadataRoot/metainfo/metfrd">
						<gmd:dateOfNextUpdate>
							<xsl:if
								test="(fn:contains(fn:lower-case(fn:normalize-space(fn:string(xs:string(.)))), 'unknown') or fn:contains(fn:lower-case(fn:normalize-space(fn:string(xs:string(.)))), 'unpublished'))">
								<xsl:attribute name="gco:nilReason">
									<xsl:sequence
										select="xs:string(xs:string(fn:lower-case(fn:normalize-space(fn:string(xs:string(.))))))"
									/>
								</xsl:attribute>
							</xsl:if>
							<xsl:variable name="var2392_cond_result_logicalor" as="xs:string?">
								<xsl:if
									test="not(((fn:contains(fn:lower-case(fn:normalize-space(fn:string(xs:string(.)))), 'unknown') or fn:contains(fn:lower-case(fn:normalize-space(fn:string(xs:string(.)))), 'unpublished')) or fn:contains(fn:lower-case(fn:normalize-space(fn:string(xs:string(.)))), 'present')))">
									<xsl:variable name="var2397_map_result_distinctvalues"
										...
										<xsl:for-each
											...
											<xsl:choose>
												...
												<xsl:if
												test="fn:exists((if (((('0' != .) and ('false' != .)) and fn:boolean(.))) then fn:substring(., xs:double(xs:decimal(0)), xs:double(fn:string-length(.))) else ()))">
												<xsl:sequence
												select="(if (((('0' != .) and ('false' != .)) and fn:boolean(.))) then fn:substring(., xs:double(xs:decimal(0)), xs:double(fn:string-length(.))) else ())"
												/>
												</xsl:if>
												</xsl:if>
												</xsl:when>
									...
									<xsl:variable name="var2396_map_result_distinctvalues"
										as="xs:string*">
										<xsl:for-each
											select="fn:distinct-values($var2397_map_result_distinctvalues)">
											<xsl:variable name="var2402_cond_result_endswith"
												as="xs:string?">
												<xsl:choose>
												<xsl:when
												test="fn:ends-with(fn:normalize-space(.), '-')">
												<xsl:if
												test="fn:ends-with(fn:normalize-space(.), '-')">
												<xsl:if
												test="fn:exists((if (((('0' != fn:normalize-space(.)) and ('false' != fn:normalize-space(.))) and fn:boolean(fn:normalize-space(.)))) then fn:substring(fn:normalize-space(.), xs:double(xs:decimal(0)), xs:double((xs:decimal(fn:string-length(fn:normalize-space(.))) - xs:decimal(0)))) else ()))">
												<xsl:sequence
												select="(if (((('0' != fn:normalize-space(.)) and ('false' != fn:normalize-space(.))) and fn:boolean(fn:normalize-space(.)))) then fn:substring(fn:normalize-space(.), xs:double(xs:decimal(0)), xs:double((xs:decimal(fn:string-length(fn:normalize-space(.))) - xs:decimal(0)))) else ())"
												/>
												</xsl:if>
												</xsl:if>
												</xsl:when>
									...
									<xsl:if test="fn:exists($var2396_map_result_distinctvalues)">
										<xsl:sequence
											select="fn:string-join($var2396_map_result_distinctvalues, ' ')"
										/>
									</xsl:if>
								</xsl:if>
							</xsl:variable>
							<xsl:for-each select="$var2392_cond_result_logicalor">
								<gco:Date>
									<xsl:sequence
										select="xs:string(xs:string(fn:string(fn:normalize-space(.))))"
									/>
								</gco:Date>
							</xsl:for-each>
						</gmd:dateOfNextUpdate>
					...
					<gmd:maintenanceNote>
						<xsl:for-each select="$var_metadataRoot">
							...
							<xsl:variable name="var2411_cond_result_exists" as="xs:string?"
								select="(if (fn:exists($var2413_map_select_metadata)) then $var2413_map_select_metadata else ' ')"/>
							<xsl:if test="fn:exists($var2411_cond_result_exists)">
								<gco:CharacterString>
									<xsl:variable name="var2412_cond_result_exists" as="xs:string"
										select="(if (fn:exists($var_metadataRoot/metainfo/metrd)) then ' Most recent metadata content review date: ' else ' ')"/>
									<xsl:sequence
										select="fn:normalize-space(fn:string(fn:concat('This metadata was automatically generated from the ', xs:string(metainfo/metstdn), ' standard, version ', xs:string(metainfo/metstdv), ' using the August 2011-REH version of the FGDC CSDGM to ISO 19115-2 transform modified and updatated by SMR 2012-02-02 to generate USGIN compatible ISO19139 XML. ', $var2412_cond_result_exists, $var2411_cond_result_exists)))"
									/>
								</gco:CharacterString>
							</xsl:if>
						</xsl:for-each>
