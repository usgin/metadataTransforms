...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  Stephen M. Richard
    ...
    2018-03-21  Version 1.1.  add 'hasPart' links to IGSN 
...
<xsl:template name="relatedSample">
    <xsl:for-each select="//*[local-name() = 'relatedIdentifier' and @relatedIdentifierType='IGSN']">
        <xsl:text>{&#10;</xsl:text>
        <xsl:text>"@type": "CreativeWork",&#10;
      "additionalType": "http://schema.geolink.org/1.0/base/main#PhysicalSample",&#10;</xsl:text>
        <xsl:text>            "identifier": {
            ...
            "additionalType": ["http://schema.geolink.org/1.0/base/main#Identifier", "http://purl.org/spar/datacite/Identifier"],
            "propertyID": "IGSN",
            "url": "</xsl:text><xsl:value-of select="concat('http://igsn.org/',string(.))"/>
        <xsl:text>",&#10;            "value": "</xsl:text>
        <xsl:value-of select="string(.)"/>
        <xsl:text>"&#10;      }&#10;}</xsl:text>
        <xsl:if test="following::*[local-name() = 'relatedIdentifier' and @relatedIdentifierType='IGSN']">
                <xsl:text>,&#10;</xsl:text>
            </xsl:if>
    </xsl:for-each>
</xsl:template>
    ...
    <xsl:template match="//*[local-name() = 'resource']">
      ...
      logic in format/profile-specific implementations to decide which to use-->
        ...
        <xsl:variable name="hasSamples" select="count(//*[local-name() = 'relatedIdentifier' and @relatedIdentifierType='IGSN']) > 0"/>
...
<!-- ******************************************************************************************* -->
    ...
    "@type":"DataCatalog",&#10;</xsl:text>
        ...
        <xsl:if test="$hasSamples">
            <xsl:text>,&#10;</xsl:text>
        </xsl:if>
        
        <xsl:if test="$hasSamples">
            <xsl:text>  "hasPart": [</xsl:text>
            <xsl:call-template name="relatedSample"/>
            <xsl:text>]</xsl:text>
